...
g <- hygiene_hypo_gen()
pub_list <- hygiene_hypo_publist_gen()
...
my.net <- network::network(x=my.edge.list, vertex.attr = pub_list, directed=TRUE)

article.type <- network::get.vertex.attribute(my.net, 'ArticleType')

article.type.convert <- case_when(
  article.type == 'empirical' ~ 0,
  article.type == 'synthesis' ~ 1
)
